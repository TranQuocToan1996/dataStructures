...
var (
	// Used to get node positions (Val, XPos, YPos)
	// Type nodePos is a struct containing Val (string) and YPos and XPos (int). Variable
	// data is defined as a slice of nodePos and is used to define the position and value of each
	// node to be graphed
	data []nodePos

	// Used to plot lines
	// Variable endPoints is defined as a slice of nodePair and is used to keep track of the end
	// point values of the lines connecting nodes in the binary tree
	endPoints []nodePair
)
...
func getEndPoints(node *Node, parent *Node) {
	if node != nil {
		if parent != nil {
			endPoints = append(endPoints, nodePair{node.Value,
				parent.Value})
		}
